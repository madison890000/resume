...
test('renders SelectLanguage', () => {
    const SelectLanguageDom = renderer.create(<SelectLanguage options={LANGUAGE_OPTIONS} />).toJSON();
    expect(SelectLanguageDom).toMatchSnapshot();
});
...
test('SelectLanguage contain English 中文 中文繁体 中文简体', async () => {
    ...
    const menuPops = screen.getByTestId('select-language-menu-pop');
    expect(menuPops).toBeVisible();
    expect(menuPops.innerHTML).toContain('中文简体');
    expect(menuPops.innerHTML).toContain('中文繁体');
    expect(menuPops.innerHTML).toContain('中文');
    expect(menuPops.innerHTML).toContain('English');
});
...
test('close SelectLanguage snapshot', async () => {
    ...
    fireEvent.click(selectBtn);
    await waitFor(() => {
        expect(screen.queryByTestId('select-language-menu-pop')).toBe(null);
        expect(document.body).toMatchSnapshot();
    });
});
